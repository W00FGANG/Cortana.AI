...
export default function TasksPage() {
  const tasks = [
    { id: "t1", title: "Follow up with qualified prospects.", agent: "Kainoa", status: "Pending", priority: "High", scheduled: "Today, 2:00 PM" },
    { id: "t2", title: "Find Hawaii businesses with outdated websites.", agent: "Kent", status: "Running", priority: "Medium", scheduled: "Today, 10:00 AM" },
    ...
    { id: "t5", title: "Send outreach email to John Doe", agent: "Kainoa", status: "Needs Approval", priority: "High", scheduled: "Today, 11:30 AM" },
  ];
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage and track all AI worker tasks.</p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {tasks.map((task) => (
                <tr key={task.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{task.agent}</td>
                  ...
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{task.scheduled}</td>
                  <td className="px-6 py-4 text-right">
                    {task.status === "Pending" || task.status === "Needs Approval" ? (
                      <button className="inline-flex items-center gap-1.5 rounded bg-blue-50 dark:bg-blue-900/30 px-2 py-1 text-xs font-medium text-blue-700 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-800/50 transition-colors">
                        <Play className="h-3 w-3 fill-current" />
                        Run Now
                      </button>
                    ) : (
                      <span className="text-slate-400 dark:text-slate-500 text-xs">No actions</span>
                    )}
                  </td>
                </tr>
              ))}
